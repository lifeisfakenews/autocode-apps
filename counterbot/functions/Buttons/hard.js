...
try {
  ...
  scores.rows.sort((a, b) => b.fields.Hard - a.fields.Hard);
  scores = scores.rows.filter((r) => r.fields.Hard > 0);
  ...
  await lib.discord.channels['@0.3.0'].messages.update({
    ...
    embeds: [{
      type: "rich",
      description: scores.map((r, i) =>  `\`${i+1<10 ? `0`:``}${i+1<100 ? `0`:``}${i+1}\` <@${r.fields.User}> ${r.fields.Hard || 0}`).slice(0, 120).join('\n'),
      color: parseInt(process.env.COLOR),
      timestamp: new Date().toISOString(),
      author: {name: `Hard Mode Leaderboard`},
    ...
    components: [{
      ...
      components: [
        ...
        {style: 1, label: `Hard`, custom_id: `hard`, type: 2},
