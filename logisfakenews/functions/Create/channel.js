...
let audit = await lib.discord.guilds['@0.2.3'].auditLogs.list({guild_id: context.params.event.guild_id, action_type: 10, limit: 1});//Get audit log data
let user = await lib.discord.users['@0.2.0'].retrieve({user_id: audit.audit_log_entries[0].user_id});//Get user daya
let permString = ``
...
**Permissions:** \n${permString}`,
      color: 0x41CF3C,
      author: {name: `Channel Created`, icon_url: user.avatar_url},
