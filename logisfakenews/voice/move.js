...
module.exports = async (event, channel, old) => {
  let audit = await lib.discord.guilds['@0.2.3'].auditLogs.list({guild_id: event.guild_id, action_type: 26, limit: 1});//Get audit log data
  let user
  ...
  await lib.discord.channels['@0.3.0'].messages.create({
    ...
    embeds: [{
      type: 'rich',
      description: `
**Previous Channel:** <#${old}>
**Current Channel:** <#${event.channel_id}>
...
${user ? `**Moderator:** ${user.username}#${user.discriminator} | <@${user.id}>` : ``}`,
      color: 0x41CF3C,
      author: {name: `User Moved VC`, icon_url: event.member.user.avatar ? `https://cdn.discordapp.com/avatars/${event.member.user.id}/${event.member.user.avatar}.${event.member.user.avatar.startsWith('a_')?'gif':'png'}` : 'https://bit.ly/3KPnEKt'},
