...
module.exports = async (event, channel) => {
  let audit = await lib.discord.guilds['@0.2.3'].auditLogs.list({guild_id: event.guild_id, action_type: 27, limit: 1});//Get audit log data
  let user
...
${user ? `**Moderator:** ${user.username}#${user.discriminator} | <@${user.id}>` : ``}`,
      color: 0xE72020,
      author: {name: `User Left VC`, icon_url: event.member.user.avatar ? `https://cdn.discordapp.com/avatars/${event.member.user.id}/${event.member.user.avatar}.${event.member.user.avatar.startsWith('a_')?'gif':'png'}` : 'https://bit.ly/3KPnEKt'},
