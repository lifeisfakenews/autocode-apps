...
if (targetAdmin)
  return lib.discord.interactions['@release'].followups.ephemeral.create({token,
    content: `<:error:${emojis.error}> | **${member.user.username}**, you can't clear a moderator's warns.`
  });
...
await lib.airtable.query['@1.0.0'].delete({
  ...
  where: [{
    'Guild__is': guild_id,
    'User__is': target.user.id,
    'Type__is': `Warning`
  }],
});
await lib.discord.interactions['@release'].followups.create({token, content: `Cleared **${target.user.username}** warnings for ${reason}`});
