...
let msgs = await lib.discord.channels['@release'].messages.list({channel_id, limit: event.data.options[0].value});
await lib.discord.channels['@release'].messages.bulkDelete({channel_id, messages: msgs.map(x => x.id)});
await lib.discord.channels['@release'].messages.create({channel_id, content: `Deleted **${event.data.options[0].value}** messages for ${reason}`});
