...
if (targetAdmin)
  return lib.discord.interactions['@release'].followups.ephemeral.create({token,
    content: `<:error:${emojis.error}> | **${member.user.username}**, you can't kick a moderator.`
  });
...
await lib.discord.guilds['@release'].members.destroy({guild_id,
  user_id: target.user.id,
  reason: `Reason ${reason} | Moderator: ${member.user.username}#${member.user.discriminator}(${member.user.id})`
});
await lib.discord.interactions['@release'].followups.create({token, content: `**${target.user.username}** has been kicked for ${reason}`});
