...
if (targetAdmin)
  return lib.discord.interactions['@release'].followups.ephemeral.create({token,
    content: `<:error:${emojis.error}> | **${member.user.username}**, you can't warn a moderator.`
  });

await lib.discord.interactions['@release'].followups.create({token, content: `**${target.user.username}** has been warned for ${reason}`});
  ...
  try {
    await lib.discord.users['@release'].dms.create({
      ...
      embeds: [{
        type: 'rich',
        title: 'Warning',
        description: `Hey! You've been warned in **${guildInfo.name}**.\n
...
<:reason:${emojis.reason}> | ${reason}`,
      ...
      author: {name: `${cNum} | Warning`, icon_url: target.user.avatar_url || 'https://cdn.discordapp.com/attachments/911294620439293993/965586325208195142/avatar.png'},
