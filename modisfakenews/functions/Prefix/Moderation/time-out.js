...
let match = event.content.split(' ')[2].match(/(\d+)(s|m|h|d)/) 
if (!match)
...
await lib.discord.channels['@release'].messages.create({channel_id, content: `**${target.user.username}** has been timed out for ${match[1]} ${match[2].replace('s', 'seconds').replace('d', 'days').replace('h', 'hours').replace('m', 'minutes')} for ${reason}`});
