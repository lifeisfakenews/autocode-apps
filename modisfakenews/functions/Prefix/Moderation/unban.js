...
let isAdmin = false, reason = event.content.split(' ').slice(2).join(' ') || `No reason provided`, user_id = event.content.split(' ')[1]
if (!user_id) throw new Error(`You must provide a user ID`);
...
let target = await lib.discord.users['@release'].retrieve({user_id});
await lib.discord.guilds['@release'].bans.destroy({user_id, guild_id});
await lib.discord.channels['@release'].messages.create({channel_id, content: `**${target.username}** has been unbanned for ${reason}`});
if (base.rows[0].fields.Logging) {
  await lib.discord.channels['@release'].messages.create({
    ...
    embeds: [{
      ...
      color: 0x41CF3C,
      description: `
<:user:${emojis.user}> | <@${target.id}> | ${target.username}#${target.discriminator}
...
<:reason:${emojis.reason}> | ${reason}`,
      ...
      author: {name: `${cNum} | Unban`, icon_url: target.avatar_url || 'https://cdn.discordapp.com/attachments/911294620439293993/965586325208195142/avatar.png'},
