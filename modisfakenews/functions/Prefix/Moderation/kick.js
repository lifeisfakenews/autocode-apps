...
try {
  ...
  await lib.discord.users['@release'].dms.create({
    ...
    embeds: [{
      type: 'rich',
      title: 'Kicked',
      description: `Hey! You've been kicked from **${guildInfo.name}**.\n
...
await lib.discord.guilds['@release'].members.destroy({user_id: target.user.id, guild_id, reason});
await lib.discord.channels['@release'].messages.create({channel_id, content: `**${target.user.username}** has been kicked for ${reason}`});
...
<:reason:${emojis.reason}> | ${reason}`,
      ...
      author: {name: `${cNum} | Kick`, icon_url: target.user.avatar_url || 'https://cdn.discordapp.com/attachments/911294620439293993/965586325208195142/avatar.png'},
