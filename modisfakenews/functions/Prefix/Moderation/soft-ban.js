...
await lib.discord.guilds['@release'].bans.create({user_id: target.user.id, guild_id, delete_message_days: 7, reason});
await lib.discord.guilds['@release'].bans.destroy({user_id: target.user.id, guild_id, reason});
await lib.discord.channels['@release'].messages.create({channel_id, content: `**${target.user.username}** has been soft banned for ${reason}`});
