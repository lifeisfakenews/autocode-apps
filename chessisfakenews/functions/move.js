...
if (context.params.event.author.id != chess.turn().replace('w', game.white.id).replace('b', game.black.id))
  return lib.discord.channels['@0.3.2'].messages.create({
    channel_id: context.params.event.channel_id,
    content: `**${context.params.event.author.username}**, it's not your turn!`
  });

let input = context.params.event.content.split(' ')
let move = chess.move({from: input[1], to: input[2]})
if (move == null) {
  if (chess.in_check())
    return lib.discord.channels['@0.3.2'].messages.create({
      channel_id: context.params.event.channel_id,
      content: `**${context.params.event.author.username}**, you are in check!`,
    });

  return lib.discord.channels['@0.3.2'].messages.create({
    channel_id: context.params.event.channel_id,
    content: `**${context.params.event.author.username}**, that move is not valid!`
  });
...
.replace('Q', '♚').replace('q', '♔')
  ...
  await lib.utils.kv['@0.1.16'].set({key: `${context.params.event.channel_id}_chess`, value: game});
} else {
  await require('../helpers/gameover.js')(context.params.event, game, chess)
}