...
let win
if (context.params.event.author.id == game.white.id) {
  win = `${game.black.username}#${game.black.discriminator}`
} else if (context.params.event.author.id == game.black.id) {
  win = `${game.white.username}#${game.white.discriminator}`
}
...
await lib.discord.channels['@0.3.2'].messages.create({
  ...
  embeds: [{
    type: "rich",
    author: {name: `${win} Won!`, icon_url: `https://cdn.discordapp.com/attachments/911294620439293993/995617018499846194/chess.png`},
    description: `♟︎  - ${game.white.username}#${game.white.discriminator}\n♙ - ${game.black.username}#${game.black.discriminator}\n
${context.params.event.author.username}#${context.params.event.author.discriminator} gave up
\`\`\`${chess.ascii()
...
await lib.utils.kv['@0.1.16'].clear({key: `${context.params.event.channel_id}_chess`});