...
await lib.discord.guilds['@release'].members.destroy({guild_id,
  user_id: target.id,
  reason: `Mod - ${event.member.user.username}#${event.member.user.discriminator}(${event.member.user.id}) ${event.data.options[1]?.value ?  `Reason - ${event.data.options[1]?.value}` : ``} `
});
await lib.discord.interactions['@release'].followups.create({token, 
  content: `**${target.username}** has been kicked ${event.data.options[1]?.value ?  `for ${event.data.options[1]?.value}` : ``}`
});
await lib.discord.channels['@release'].messages.create({
  ...
  embeds: [{
    ...
    author: {name: `${caseNum.count + 1} | Kick`, icon_url: target.avatar_url || 'https://bit.ly/3KPnEKt'},
