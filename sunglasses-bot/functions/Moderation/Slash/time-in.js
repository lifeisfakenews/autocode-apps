...
await lib.discord.guilds['@release'].members.timeout.destroy({user_id: target.id, guild_id});
await lib.discord.interactions['@release'].followups.create({token, 
  content: `**${target.username}** has been timed in ${event.data.options[1]?.value ?  `for ${event.data.options[1]?.value}` : ``}`
});
await lib.discord.channels['@release'].messages.create({
  ...
  embeds: [{
    type: 'rich',
    color: parseInt(process.env.GREEN),
    ...
    author: {name: `${caseNum.count + 1} | Time In`, icon_url: target.avatar_url || 'https://bit.ly/3KPnEKt'},
