...
await lib.discord.interactions['@release'].followups.create({token, 
  content: `**${target.username}** has been unbanned ${event.data.options[1]?.value ?  `for ${event.data.options[1]?.value}` : ``}`
});
await lib.discord.channels['@release'].messages.create({
  ...
  embeds: [{
    ...
    author: {name: `${caseNum.count + 1} | Unban`, icon_url: target.avatar_url || 'https://bit.ly/3KPnEKt'},
