...
let info = await lib.http.request['@1.1.5'].get({
  url: `http://api.weatherapi.com/v1/current.json?key=${process.env.WEATHER_API_KEY}&q=${context.params.event.message.embeds[0].author.name.split('In ')[1]}&aqi=no`,
});
...
await lib.discord.interactions['@release'].responses.update({
  token: context.params.event.token,
  response_type: 'UPDATE_MESSAGE_WITH_SOURCE',
  content: ``,
  embeds: [{
    type: 'rich',
    description: `
...
🌡️ ${info.data.current.temp_f}°F
🥵 ${info.data.current.feelslike_f}°F
🍃 ${info.data.current.wind_mph} mph
...
.replace('s', 'South')
.replace('w', 'West')}`}
${info.data.current.precip_in > 0 ? `💧 ${info.data.current.precip_in} in\n` : ``}💦 ${info.data.current.humidity}%
☀️ ${info.data.current.uv}`,
  ...
  components: [
    {type: 1, components: [
      {style: 2, label: `Metric`, custom_id: `weather-metric`, type: 2},
      {style: 1, label: `Imperial`, custom_id: `weather-imperial`, type: 2}
    ]}
  ],
});